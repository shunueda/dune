...
let to_dyn { url = _loc, url; checksums } =
  Dyn.record
    ...
    ; "checksum", Dyn.list (fun (_loc, checksum) -> Checksum.to_dyn checksum) checksums
    ]
;;
...
let compute_missing_checksum
  ...
  | [] ->
    ...
    >>| Options.map ~f:(fun checksum ->
      { url = url_loc, url; checksums = Some (Loc.none, checksum) })
...
let decode_fetch =
  ...
  and+ checksum = field_o Fields.checksums (located string) in
  let checksums =
    match checksum with
    | None -> None
...
let encode_fetch_field { url = _loc, url; checksums } =
  ...
  ; field_o Fields.checksum Checksum.encode (List.map checksums ~f:snd)
  ]
;;
